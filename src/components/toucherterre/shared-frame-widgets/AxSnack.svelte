...
>
	<Label>{$AX_STORE__FRAME_snack.text}</Label>
	<Actions>
		<IconButton
...
<script lang="ts">
	...
	//
	onMount(() => {
		axlog({ data: $AX_STORE__CONTENT, db: $AX_STORE__CONTENT_DB }, $page.url.pathname, 'wc -- ax snack', false, 'MOUNT', true);
	});
	//
	$: if ($AX_STORE__FRAME_snack.open && snackbar) {
		snackbar.open();
	}
